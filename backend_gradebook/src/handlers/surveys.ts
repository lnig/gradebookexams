...
export const sendSurvey = async (req: Request, res: Response) => {
    try {
      ...
      const surveyQuestions = await prisma.questions.findMany({
        where: {
          survey_id: Buffer.from(uuidParse(surveyId))
        }
      });
  
      if (surveyQuestions.length !== responses.length) {
        return res.status(400).json(createErrorResponse("Survey is not complete."));
      }
      ...
      for (const response of responses) {
        ...
        if (existingQuestion.questions_types.name === "Closed") {
          ...
          if (!answerExist) {
            return res.status(400).json(createErrorResponse("Answer does not exist."));
          }
        }
      ...
      return res.status(200).json(
        createSuccessResponse(payload.count, "Survey submitted successfully.")
      );
    } catch (err) {
      return res.status(500).json(createErrorResponse("An unexpected error occurred while submitting survey. Please try again later.")
        );
    }
  };

  export const getSurveyFullDetails = async (req: Request, res: Response) => {
    try {
      const surveyId: string = req.params.surveyId;
      ...
      const processedQuestions = existingSurvey.questions.map(question => ({
        ...
        questions_responses: question.questions_responses.map(response => ({
          ...response,
          id: uuidStringify(response.id),
          student_id: uuidStringify(response.student_id),
          question_id: uuidStringify(response.question_id)
        }))
      }));
      ...
      return res.status(200).json(createSuccessResponse(responseData, `Survey details retrieved successfully.`));
    } catch (err) {
      console.error('Error retrieving survey details', err);
      return res.status(500).json(createErrorResponse('An unexpected error occurred while retrieving survey details. Please try again later.'));
    }
    };
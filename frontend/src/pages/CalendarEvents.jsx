...
export function CalendarEvents() {
  ...
  const fetchEvents = async (currentUserId) => {
    if (userRole !== UserRoles.Administrator && !currentUserId) return;
  ...
  const handleConfirmEventDelete = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/school-event/${eventToDelete.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const handleConfirmExamDelete = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/exam/${examToDelete.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
        ...
        <div className='flex flex-col w-full'>
          ...
          <div className='flex flex-col gap-4 xl:flex-row xl:gap-16'>
            <div className="flex flex-col xl:w-80">
              ...
              <div className="mt-6 bg-white w-full xl:w-80">
                        ...
                        return (
                          <div key={idx} className='flex justify-between items-center border border-textBg-200 w-full p-3 rounded'>
                            <div className="flex gap-4">
                              <div className={`hidden w-12 h-12 ${eventTypeCardColors[event.event_type_id] || 'bg-gray-500'} rounded md:flex items-center justify-center`}>
                              ...
                              {event.type === 'exam' && (
                                <>
                                  <Button
                                    icon={<Pen size={16} color="#1A99EE" />}
                                    type="link"
                                    onClick={() => openEditExamModal(event)}
                                  />
                                  <Button
                                    icon={<Trash size={16} />}
                                    type="link"
                                    onClick={() => openDeleteExamModal(event)}
                                  />
                                </>
                              )}
                            </div>
                          ...
                          {userRole === UserRoles.Teacher && event.type === 'exam' && (
                            <div className="flex">
                              <Button
                                icon={<Pen size={16} color="#1A99EE" />}
                                type="link"

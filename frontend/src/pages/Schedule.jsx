...
export function Schedule() {
  ...
  const events = mapLessonsToEvents(lessons);
  ...
  const handleLessonUpdate = async ({ lessonId, lessonTopic }) => {
    ...
    try {
      ...
      toast.success(updatedLesson.message || 'Lekcja zaktualizowana pomyślnie.');
    } catch (err) {
      toast.error(err.message || 'An unexpected error occurred.');
    } finally {
      setUpdating(false);
  ...
  const openDropdown = (event, buttonElement) => {
    ...
    let left = rect.left + window.scrollX - dropdownWidth; 
                    ...
                    <div className="relative overflow-hidden" style={{ height: `${calendarHeight}px` }}>
                      ...
                      .map((event, eventIdx) => {
                        ...
                        return (
                          <div
                            key={eventIdx}
                            className={`absolute right-[2px] p-2 rounded-sm ${event.colors.bg} ${event.colors.text} ${userRole === UserRoles.Teacher || userRole === UserRoles.Administrator ? 'cursor-pointer' : ''}`}
                          ...
                          >
                            <div className='flex justify-between'>
                              ...
                              <div className='flex items-center z-10'>
                                ...
                                >
                                  <Info 
                                    className={`w-4 h-4 ${event.colors.text} cursor-pointer`} 
                                    strokeWidth={3} 
                                ...
                                {(userRole === UserRoles.Teacher || userRole === UserRoles.Administrator) && (
                                  <div className='hidden md:flex -space-x-1'>
                                    <Button 
                                      type="link" 
                                      icon={<Award size={16} strokeWidth={3} className={`${event.colors.text}`}/>} 
                                      size="xs"
                                      className="z-10" 
                                      onClick={(e) => {
                                        e.stopPropagation();
                                    ...
                                    <Button 
                                      type="link" 
                                      icon={<FilePlus2 size={16} strokeWidth={3} className={`${event.colors.text}`}/>} 
                                      size="xs"
                                      className="z-10" 
                                      onClick={(e) => {
                                        e.stopPropagation();
                                    ...
                                    <Button 
                                      type="link" 
                                      icon={<NotebookPen size={16} strokeWidth={3} className={`${event.colors.text}`}/>} 
                                      size="xs"
                                      className="z-10" 
                                      onClick={(e) => {
                                        e.stopPropagation();
                        ...
                        {events
                          ...
                          .map((event, eventIdx) => {
                            ...
                            return (
                              <div
                                key={eventIdx}
                                className={`absolute mx-auto p-2 rounded-sm ${event.colors.bg} ${event.colors.text} cursor-pointer`}
                                style={{
                                  top: `${top + 2}px`,
                                  height: `${height - 5}px`,
